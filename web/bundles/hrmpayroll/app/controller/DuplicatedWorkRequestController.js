 ...
 */
Ext.define('sisprod.controller.DuplicatedWorkRequestController', {
   extend: 'sisprod.controller.Base',
   stores : ['DuplicatedWorkRequestStore'],
   models : ['DuplicatedWorkRequestModel'],
   entityName: 'DuplicatedWorkRequest',
   checkOutPermissions: false,
   refs: [{ref: 'listDuplicatedWorkRequest', selector: 'listDuplicatedWorkRequest'}],
   views : ['DuplicatedWorkRequest.ListDuplicatedWorkRequest'],
   
   requires: [
       'sisprod.store.DuplicatedWorkRequestStore'
   ],
   ...
   init : function(){
        this.control({
           'listDuplicatedWorkRequest button[action=nullify]':{
               click: this.nullify
           },
           'listDuplicatedWorkRequest button[action=viewDetail]':{
               click: this.showTaskDescription
           },
           'listDuplicatedWorkRequest dataview': {
               itemdblclick: this.showTaskDescription
           },
           'nullifyDuplicatedWorkRequest button[action=save]': {
                ...
                success: function(response){
                    ...
                    if(Ext.isDefined(responseData.success) && responseData.success === true){
                        if(responseData.result){
                            var nullifyWindow = Ext.create('sisprod.view.DuplicatedWorkRequest.NullifyWorkRequest',{
                                record: record
                            });
                            nullifyWindow.show();
                        }
   ...
   releaseNullify: function(button){
    ...
    getGridForEntity: function(){
        var tabGrid = this.getListDuplicatedWorkRequest();
